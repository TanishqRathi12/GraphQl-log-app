...
export const page = async({searchParams}:{searchParams: Promise<{search:string}>}) => {
    const data = await searchParams;
    const query = data.search
    const res : { BlogArr: Blog[] } = await gqlClient.request(GET_SEARCH,{
        searchQuery:query
    })
    const blog = res.BlogArr;
  return (
    <>
    <form action={"/search"} className="flex items-center justify-center mb-6 mt-6">
      <input
      type="text"
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-4">
      {blog.length > 0 ? blog.map((item: Blog) => (
      <div
        key={item.id}
      ...
      >
        ...
        <Link href={`blog/${item.id}`}>
          <h2 className="text-xl hover:underline text-black font-semibold mb-2">{item.title}</h2>
        </Link>
